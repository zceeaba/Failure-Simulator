...
from churnsim.uk.ac.bristol.rechurn.modes.CloudFailures.DistributionBased.Gossip import GossipNode
from churnsim.uk.ac.bristol.rechurn.modes.CloudFailures.DistributionBased.Task import Task
...
class SoftwareFailures(FailureMode):
    ...
    def generateworkloadsizes(self,size):
        ws = [Task(i, random.randint(0, 100)) for i in range(0, size)]
        return ws
    ...
    def get_new_topology(self, topology):
        ...
        capacities = [random.randint(50,100 ) for j in range(0, len(new_topology.nodes))]
